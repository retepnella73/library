class PublishersController < ApplicationController
  ...
  # GET /publishers.json
  def index
    @publishers = Publisher.order('sort_name').page(1).per(10)

    respond_to do |format|
      format.html # index.html.erb
      format.json { render json: PublishersDatatable.new(view_context, 
                                                         (permitted_to? :edit, @publisher), 
                                                         (permitted_to? :destroy, @publisher)) }
      format.js
    end
  end
